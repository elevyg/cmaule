...
const ValoresContainer = () => {
  return (
    <Fade direction="bottom" duration={3000}>
      <div
        id="valores"
        className="flex justify-between items-center py-5 w-screen flex-grow-0 overflow-hidden"
      >
        <div id="valores-izq ">
          <Valores
        ...
        <Valores
          className="text-left  "
          valores={["Puentes", "Ciclovias", "Pasos sobrenivel", "Hormigón", ""]}
        />
      </div>
    </Fade>
  )
    ...
    {valores &&
      valores.map((valor, index) => (
        <div className="relative">
          <motion.p
            key={valor}
            className=" text-lg md:text-3xl text-gray-800 z-50 bold absolute"
            whileHover={{ scale: 1.3 }}
            style={
              {
                // top: `-${(index + 1) * 3}rem`,
                // bottom: `${index + 1}rem`,
                // left: `-${(index + 1) * 0.1}rem`,
                // right: `-${index}rem`,
              }
            }
          >
