    ...
    {valores &&
      valores.map((valor, index) => (
        <div className="flex relative">
          <motion.div
            className="h-32 w-32 absolute bg-dark-blue rounded-full z-40"
            style={
              (index === 0
                ? { top: 10, bottom: 0, left: 50, right: 0 }
                : { top: 70, bottom: 0, left: 100, right: 0 },
              { opacity: 0.7 })
            }
            whileHover={{ scale: 1.5 }}
          >
            <div
              className="h-20 w-20 absolute bg-yellow-400 rounded-full z-10"
              style={
                (index === 0
                  ? { top: -20, bottom: 30, left: -30, right: -100 }
          ...
          <motion.p
            key={valor}
            className=" text-xl md:text-5xl my-12 relative top text-gray-800 z-50"
